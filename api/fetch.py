...
class RecipeExtractor(HTMLParser):
    """Haalt leesbare tekst uit HTML, filtert rommel weg."""
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/health":
            self.send_json(200, {"status": "ok", "service": "Rijnsburgje proxy"})
            return
        ...
        if not url_param:
            self.send_json(400, {"error": "Geen URL meegegeven (?url=...)"})
            return

        url = unquote(url_param)

        if not url.startswith("http"):
            self.send_json(400, {"error": "Ongeldige URL (moet beginnen met http)"})
            return
